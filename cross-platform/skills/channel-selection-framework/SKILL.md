...
description: |
  Comprehensive decision framework for selecting advertising channels based on campaign objectives,
  audience characteristics, budget constraints, and platform strengths. Includes platform comparison
  matrices, budget threshold guidelines, audience fit analysis, and funnel stage recommendations.

  Use when: user asks about channel selection, platform comparison, which ads to run where, media mix
  decisions, budget allocation across platforms, when to use Meta vs Google vs LinkedIn vs TikTok,
  or channel strategy for new campaigns.
---
